...
def Studies_PlotMain():
    ...
    parser=argparse.ArgumentParser(description=__doc__);selection=parser.add_mutually_exclusive_group();selection.add_argument('--fig',choices=names)
    ...
    if args.fig and args.gif_only:parser.error('--fig cannot be combined with --gif-only')
    ...
    if not args.gif_only and not args.gif:
        for name in [args.fig] if args.fig else names:
            try:
    ...
    if (args.fig or args.gif or args.gif_only) and history.exists():
