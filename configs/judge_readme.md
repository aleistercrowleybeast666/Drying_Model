...
完整开发审计（最重）：保留 thermal 18 条严格验证、12 条守恒、完整二维辅助、所有 GIF 和 legacy 诊断，耗时显著更长。强制参考是独立选项，不自动勾选。
...
命令行示例：
    ...
    药材烘干模型_原题表格复算.exe --developer-full-audit --force-reference --explain-schedule
    药材烘干模型_原题表格复算.exe --all --dry-run
    药材烘干模型_原题表格复算.exe --verify

--workers 默认为 auto，可设 1～6。auto 使用物理核数减一、最多 6 个 CPU token；内存预算为启动时可用内存的 72%，还须保留至少 2 GiB。二维/一维都占一个 CPU token，按历史 RSS 加余量调度，子进程内线程数均为 1。二维子验证独立调度，发布只锁冲突文件；一维实验及守恒复用 persistent worker。数值内核保持原样。
...
运行记录：work/recompute/timings.json、results/recompute_timing_summary.json；开发全量另有 results/developer_full_audit_timing.json，列出启动、求解、CPU、RSS、关键路径与并发统计。
...
GUI“全选（论文复现）”不选动画及 thermal 的 18 条 supplemental 验证；未选择的验证明确显示 NOT_SELECTED_SUPPLEMENTAL，不能冒充 PASS。“完整开发审计（最重）”保留全部 18 条。当前论文明确引用 12/12 守恒，因此论文复现保留 12 例并按 production 完成后流水审计；冷启动属于较长的严格检查，不冒充已经验证。高级区域可选动画、强制参考和历史开发诊断。
D 的一维分支不等待二维辅助汇总；仅 matched/end-effect 和最终发布等待二维。B/D 三条同身份 M00 全程参考只算一次；D.full.q1 经规格与正式参考检查后引用完整 1800 s 的 A.q1。72 h 共同对照仍需要的轨迹共享最长一次，未擅自缩短证据时域。
...
构建/验收细节及未实测项目见 dependencies/application/code/RELEASE_V4_VALIDATION.md。
