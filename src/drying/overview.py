...
def Overview_Render(data):
    ...
    for case in ['q1','q23','q4']:
        ...
        stats = '; '.join(' / '.join(fmt(s.get(k),5) for k in ['actual_dt_min','actual_dt_mean','actual_dt_max']) for s in stages)
        ...
        if not entry.get('spatial_passed'):
            warnings.append(case+': SPATIAL_CONVERGENCE_FAILED（固定网格空间误差仍超限；当前结果为候选）')
    ...
    lines += ['', '| 问题 | 求解 / 时间 / 空间 / 阶段敏感性 | 干燥时间 h | 终点 Cmax | Excel |',
              '|---|---|---|---|---|']
    ...
    for case in ['q1','q23','q4']:
        ...
        for trial in entry.get('stage_schedule_trials',[]):
            ...
            trials.append(('160→80' if 'conservative' in trial['schedule_id'] else '160→80→40')+': '+
        ...
        for attempt in attempts:
            if attempt['case_id'] != data['sources'].get(case+'_1d',{}).get('case_id'):
                stats = '; '.join(f'nr{s["nr"]}: '+ '/'.join(fmt(s[k],5) for k in
                    ['actual_dt_min','actual_dt_mean','actual_dt_max']) for s in attempt['stage_dt_statistics'])
                projection_lines.append(f'- {case} 未采用阶段方案 dt min/mean/max（s）：{stats}。')
    tol = cfg['validation']['spatial']
    lines += ['',*projection_lines,'',f'阈值：ΔT≤{tol["temperature_abs_K"]} K、ΔC≤{tol["moisture_abs"]} kg/kg、干燥时间相对差≤{100*tol["drying_time_relative"]:g}%。阶段 PASS 只约束附加误差，不替代固定网格空间收敛。','',
    ...
    lines += ['', '一维/二维差值仍包含空间离散误差，不能单独视为纯端面效应。',
