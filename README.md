...
洁净提交目录为 `release_v5_submit/A题_药材烘干模型/`；当前人工测试目录 `release_v5/` 保留不动，V2/V4 同样保持不变。终端版只生成 A 四张正式表；完整 GUI 版提供 A/B/C/D，两套依赖、源码和运行目录完全独立。

用户已完成四表终端版实际运行，本次测试电脑约 2.8 min，仅供参考。终端新增只读结构进度和 ETA：读取正式阶段及 stagepart 已写出的状态，按阶段时长与径向网格计算显示工作量；状态不更新时百分比不自行增长。ETA 优先采用科学身份、电脑和任务范围匹配的成功历史，首次运行根据实际进度自校准。新增显示未重新完整跑四表，不将用户既有数值实测当作新 ETA 的完整实测。

运行 `python scripts/finalize_release_v5.py` 生成新的洁净提交包；不覆盖正在测试的发布目录，提交包不携带 results/work/logs。GUI 程序保持字节不变，只更新提交副本的说明。静态完整性和开发验收记录见 [RELEASE_V5_VALIDATION.md](RELEASE_V5_VALIDATION.md)。

GUI 新增“刷新缓存”：按每一项依赖核验跨运行缓存。已完成问题1和问题2/3时，对应 C/D 可以选择；未完成问题4只禁用依赖问题4的项目。缓存失效会禁用并取消相应勾选，全选不会选择禁用项。详细中文选择、命令和内存策略见 [V5 使用说明](configs/judge_readme_v5.md)。

完整 GUI 后台的 `--paper-all` 为 A+B；`--all` 明确安排 A/B 完成后再执行 C/D；`--developer-full-audit` 为 D，不补生产。四表终端版不提供这些扩展参数。数据身份及文件哈希在启动前再次核验。旧版本说明和历史实测见下文，其任务分组不适用于 V5。
