...
最新发布目标是 `release_v5/`，V2/V4 发布包保持不变。默认仅 A 四张正式表，B 逐实验生产数据，C 逐图读取缓存，D 独立验证。
...
CLI `--paper-all` 为 A+B；`--all` 明确安排 A/B 完成后再执行 C/D；`--developer-full-audit` 为 D，不补生产。数据身份及文件哈希在启动前再次核验。旧版本说明和历史实测见下文，其任务分组不适用于 V5。
