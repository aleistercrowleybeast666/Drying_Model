...
最终目录为 `release_v5_final/A题_药材烘干模型/`。终端版与 GUI 版各有独立 `main.py`、`dependencies/`、`requirements.txt`、`安装依赖.bat`、`results/work/logs` 和完整的 `原始题目数据/`，任一版本可单独复制使用。推荐 Python 3.11（64 位），先安装对应目录依赖，再运行 `python main.py`。

最终准备入口是 `python scripts/prepare_release_v5_final.py`，只整理源码与核验有效的数据，不生成 EXE。原 `release_v5` 人工测试目录和 V2/V4 历史目录保持不动。两版原始 PDF、附件共各 7 个文件逐文件 SHA256 相同，输入数值 hash 与预处理数组不变。GUI 从本版“原始题目数据”准备输入，仓库模式兼容 `data/raw`；终端直接使用预处理输入，只准备填表模板。

终端沿用 da351c0 的真实结构进度和 ETA，`official_progress.py` 字节不变。用户测试机完整四表约 2.8 min，仅供参考。Q3=57.6215 h、Q4=51.1824 h 与冻结参考一致，数值核心、阶段网格、步长和 Excel 格式不变。

GUI 的 ETA 未知时结构百分比仍更新，历史二维成本兼容 V5 名称；顶部“全选（A+B+C+D）”明确安排全部任务，先 A/B 再 C/D。普通手动选择及模式内全选继续按逐项缓存状态控制。三个联合 GIF 的依赖已修正，D 二维证据完整封印/迁移，缺失证据拒绝 assess；同生产源的 thermal/cross 分析加互斥锁。

本次迁移有效 B 数据 24 项、重新封印 C 图 12 项；A 源 manifest 的 3 项哈希失配，按规则跳过，预置的四份 Excel 已独立通过冻结参考 readback。D 仅迁移当前身份下有效的验证 artifact，本次可迁移数量为 0；不把数据或图片存在视为验证 PASS。两个 logs 为空，终端 work 为空，首次终端运行仍计算并核验四表。详细目录、依赖与必要短检查记录见 [RELEASE_V5_VALIDATION.md](RELEASE_V5_VALIDATION.md)。

以下为历史版本与底层接口说明，不作为 V5 最终源码发布入口。

# 评委版 V4

历史 V4 发布目录为 `release_v4/A题_药材烘干模型/`。V2 的程序、源码与运行保持不动。用户已授权删除 V3 发布目录，核验无活动进程后已删除；工作区旧验收记录仅作对照。
