...
plot.py 生成五个 GIF：原有两张真实二维场的 3D 动画、一张 q3/q4 过轴截面、一张一维径向解映射的圆形截面，再追加真实二维场旋转展开的斜切圆柱。竖直圆柱在模型中部被与轴线成约 45° 的横向斜平面截断，显示下段完整圆周外壁及椭圆斜截面；隐藏上段仅为露出内部，不修改模型轴向长度和二维场。平面角度、方位角及轮廓线宽在 display 中配置。Q4 直径与椭圆边界一起真实收缩。斜切 GIF 按独立二维模型各自的烘干终点播放；正式 Excel 的终点取自一维生产方案。综合 GIF 均为左上 Q3 温度、右上 Q4 温度、左下 Q3 含水率、右下 Q4 含水率。三张综合动画使用相同相对进度，各问显示实际物理时间，前 6% 过程放慢；温度和含水率分别固定为 28–53 ℃、0–2.55 kg/kg。第四问在固定坐标范围内显示真实半径收缩。动画逐帧编码并完整解码检查。`animate --case q23` 或 `--case q4` 只重画对应 3D GIF；综合动画用不带筛选的 `animate`。
...
一维空间验证完成全过程 40→80→160，以官方物理半径（含真实表面）最大 ΔT≤0.01 K、ΔC≤0.01 kg/kg，以及烘干时间相对差≤0.2% 验收。全场 L∞、体积加权 L2 另存诊断。正式严格判断使用所有接受步端点的官方空间点（含真实表面）最大误差，并要求 E(80→160)<E(40→80)；第二轮通过选 80，否则选 160 候选。另以局部固定 400 验证早期窗口并检查 200→400 下降趋势，不宣称完整时间域的 400 收敛。数值差值不是物理误差上界。时间验证保留实际步序减半、原阈值及必要时再次减半；0.25 s 明确定义为 dt_max 上限；保留按各 RK 阶段稳定性估计进行二分保护的既定算法，实际步长不超过稳定上限。每阶段 min/mean/max 均写入 status.json 和 overview.md，短尾步计入统计。
...
只重画本轮斜砍 GIF 并更新 PNG：`.\.venv\Scripts\python.exe plot.py --gif cutaway`。其余四个 GIF 原样保留。不加参数时仍可从独立 payload 重画全部图；两种方式均不导入或调用 PDE 求解器。最终 results 中不额外输出斜砍静态 PNG。
