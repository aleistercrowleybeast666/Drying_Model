...
def Build_ArchiveTarget(target):
    ...
    if not target.is_relative_to((ROOT/'release_v4').resolve()):raise ValueError('OLD_RELEASE_PROTECTED: only release_v4 may be replaced')
...
def Build_GetReadme(facts):
    timing_path = ROOT/'work/release_v4/progress_cold_benchmark.json'
    if not timing_path.exists():timing_path = ROOT/'work/release_v4/cold_benchmark.json'
    ...
    benchmark = ('EXE 无缓存原题四表 %.2f min（%s slots，含进程启动/准备/JIT/Excel/readback）' % (timing['external_wall_s']/60,timing.get('workers','auto'))
        if timing.get('returncode') == 0 else 'V4 冷启动未实测；既有 V2 原题四表冷跑约 3.10 min。')
    warm_path=ROOT/'work/release_v4/progress_warm_benchmark.json'
    if warm_path.exists():
...
def Build_StageCode(target):
    ...
    for name in ['README.md','requirements-build.txt','requirements.txt','pytest.ini','RELEASE_VALIDATION.md','RELEASE_V4_VALIDATION.md']:
...
def Build_Run():
    ...
    parser.add_argument('--dist-dir',type=Path,default=ROOT/'release_v4',help='v4 发布目录；旧 release 禁止修改')
    args = parser.parse_args(); args.dist_dir=args.dist_dir.resolve()
    target = args.dist_dir/NAME
    if not target.resolve().is_relative_to((ROOT/'release_v4').resolve()):
        raise RuntimeError('OLD_RELEASE_PROTECTED: use --dist-dir release_v4; release_v2 is protected; old release is running')
    folder = ROOT/'work/release_v4/build'; folder.mkdir(parents=True,exist_ok=True)
